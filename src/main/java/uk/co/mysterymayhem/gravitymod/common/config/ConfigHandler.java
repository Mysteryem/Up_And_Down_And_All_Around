...
public class ConfigHandler {
    public enum EnumItemStackUseCompat {
        BLOCK(
                ...
                "\tSpecifically when the item's \"onItemUse\" method is called.\n"
        ),
        GENERAL(
    ...
    private static final String MAIN_COMMENT = "Up And Down changes how player motion is used to move the player. Some\n" +
            "\titems may move the player in unexpected ways when used. If that's the case, try adding them to one of the\n" +
            "\tbelow lists. The format is (text in square brackets is optional)\n\n" +
            ...
            "\tback to normal, taking into account any changes to motion, _but not taking into account any changes to\n" +
            "\trotation_)\n." +
    ...
    public static void loadConfig(FMLPreInitializationEvent event) {
        ConfigHandler.forgeConfig = new Configuration(event.getSuggestedConfigurationFile());
    }

    public static void processConfig() {
        Configuration config = ConfigHandler.forgeConfig;
        config.load();
    ...
    private static void processStringList(EnumItemStackUseCompat compatType) {
        String[] stringList = ConfigHandler.forgeConfig.getStringList(
